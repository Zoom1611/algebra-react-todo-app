    ...
    <div>
      <div>
        {todoState.map((item, index) => 
          <div key={index}>
            <button type="button" onClick={handleOnToggle(item,index)}>{item.isDone ? "Mark as todo" : "Mark as done"}</button>
            {item.text} <button type="button" onClick={handleOnDelete(item, index)}>Delete</button>
          </div>
        )}
      </div>
